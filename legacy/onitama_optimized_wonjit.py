...
# @njit
def offset_to_dr_dc(offset):
    """Convert single offset to (dr, dc)"""
    if offset == 0:
        return 0, 0
    dr = offset // 5
    dc = offset % 5
...
def get_possible_moves(card_idx, from_square, is_p1):
    ...
    for i in range(CARD_MOVE_COUNTS[card_idx]):
        offset = factor * CARD_OFFSETS[card_idx, i]
        dr, dc = offset_to_dr_dc(offset)
...
def make_move(p1_bb, p2_bb, p1_cards, p2_cards, extra_card, wk_pos, bk_pos, is_p1, move):
    ...
    # Undo info
    undo_info = (captured_piece, captured_king, card_idx, old_card)
    
    return new_p1_bb, new_p2_bb, new_p1_cards, new_p2_cards, new_extra, new_wk_pos, new_bk_pos, undo_info
...
def evaluate_position(p1_bb, p2_bb, p1_cards, p2_cards, extra_card, wk_pos, bk_pos, is_p1):
    ...
    wp_count = count_bits(p1_bb) - (1 if wk_pos != -1 else 0)
    bp_count = count_bits(p2_bb) - (1 if bk_pos != -1 else 0)
...
def minimax(p1_bb, p2_bb, p1_cards, p2_cards, extra_card, wk_pos, bk_pos, is_p1, 
    ...
    if depth == 0 or is_terminal(p1_bb, p2_bb, wk_pos, bk_pos):
        ...
        return score, np.array([-1, -1, -1], dtype=np.int64)
    ...
    if is_p1:
        ...
        for i in range(move_count):
            move = moves[i]
            new_p1_bb, new_p2_bb, new_p1_cards, new_p2_cards, new_extra, new_wk_pos, new_bk_pos, undo_info = make_move(
                p1_bb, p2_bb, p1_cards, p2_cards, extra_card, wk_pos, bk_pos, is_p1, move)
            
            score, _ = minimax(new_p1_bb, new_p2_bb, new_p1_cards, new_p2_cards, new_extra, 
                              new_wk_pos, new_bk_pos, False, depth - 1, alpha, beta)
        ...
        for i in range(move_count):
            ...
            score, _ = minimax(new_p1_bb, new_p2_bb, new_p1_cards, new_p2_cards, new_extra,
                              new_wk_pos, new_bk_pos, True, depth - 1, alpha, beta)
...
def main():
    ...
    p1_cards = np.array([0, 1], dtype=np.int64)  # Tiger, Dragon
    p2_cards = np.array([2, 3], dtype=np.int64)  # Crab, Elephant
    extra_card = 4  # Monkey
    wk_pos = 2
    bk_pos = 22
    is_p1 = True
    ...
    while not is_terminal(p1_bb, p2_bb, wk_pos, bk_pos):
        ...
        score, best_move = minimax(p1_bb, p2_bb, p1_cards, p2_cards, extra_card, 
        ...
        # Make the move
        p1_bb, p2_bb, p1_cards, p2_cards, extra_card, wk_pos, bk_pos, _ = make_move(
